...
export default function NearbyFacilities({ compact = false }) {
  ...
  return (
    <div style={{ marginTop: compact ? 14 : 24 }}>
      {!compact && (
        <>
          <h3 className="display" style={{ fontSize: 19, color: "var(--rose-deep)" }}>Nearest hospitals</h3>
      ...
      {locStatus === "error" && (
        <div style={{ marginTop: 12, padding: 12, background: "var(--emergency-bg)", borderRadius: 10, fontSize: 13.5, color: "var(--emergency-ink)" }}>
          {locError}
        </div>
      )}
      ...
      {coords && (
        <div style={{ marginTop: 14, padding: 14, background: "var(--cream-dim)", borderRadius: 12 }}>
      ...
      {listStatus === "searching" && (
        <p style={{ marginTop: 12, fontSize: 13, color: "var(--ink-muted)" }}>Looking up exact distances...</p>
      )}
