...
export default function CycleTracker() {
  ...
  return (
    <div className="container" style={{ paddingTop: 44, paddingBottom: 60, maxWidth: 640 }}>
      ...
      <p style={{ color: "#6B5A5F", marginTop: 8 }}>{t("cycle_subtitle")}</p>

      <div className="card" style={{ marginTop: 22 }}>
        <label style={{ fontSize: 13.5, fontWeight: 600, color: "#7E2F44" }}>Week</label>
        <input type="number" min="1" max="42" value={week} onChange={(e) => setWeek(e.target.value)}
          style={{ display: "block", marginTop: 8, width: 100, padding: "8px 10px", borderRadius: 10, border: "1px solid #F0DFE2" }} />

        <p style={{ fontSize: 13.5, fontWeight: 600, color: "#7E2F44", marginTop: 20 }}>Any of these this week?</p>
        ...
        {anyDanger && (
          <div style={{ marginTop: 16, padding: 14, background: "#FBE4E1", borderRadius: 12, color: "#7A241D", fontSize: 14 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {entries.map((e, i) => (
              <div key={i} className="card" style={{ padding: 14 }}>
                ...
                <span style={{ color: "#9C8A8F", fontSize: 12.5, marginLeft: 10 }}>{e.date}</span>
                {e.dangerSigns.length > 0 ? (
                  <p style={{ fontSize: 13, color: "#B0342C", marginTop: 6 }}>
                    Flagged: {e.dangerSigns.join(", ")}
                  </p>
                ) : (
                  <p style={{ fontSize: 13, color: "#4A7A6B", marginTop: 6 }}>No danger signs reported.</p>
                )}
              </div>
            ))}
          </div>
