...
export default function HealthReport() {
  ...
  const facts = useMemo(() => {
    const f = new Set();
    const { triage, anaemia, cycle } = report;
  ...
  // AI plain-language summary (point 11). Optional + free; the report prints
  // fine without it. Generated on demand, then rendered inside the document so
  // it prints with everything else.
  ...
  async function generateSummary() {
    ...
    try {
      const res = await api.reportSummarize(buildAiContext(report));
      setAi(res);
    } catch (err) {
      if (err?.status === 503) setAiError("AI summary isn't enabled on the server yet (needs a free key).");
      else setAiError(err?.message || "Could not write the summary. Please try again.");
    } finally {
      setAiBusy(false);
    }
  }
  ...
  return (
    <div className="container" style={{ paddingTop: 30, paddingBottom: 60 }}>
      ...
      {/* AI summary control — never prints. */}
      <div className="no-print" style={{ margin: "6px 0 14px", display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
        <button className="btn" onClick={generateSummary} disabled={aiBusy}>
          <Icon name="document" size={16} /> {aiBusy ? "Writing…" : ai ? "Rewrite plain-language summary" : "Add a plain-language summary (AI)"}
        </button>
        {aiError && <span style={{ color: "var(--emergency)", fontSize: 13 }}>{aiError}</span>}
      </div>
      ...
      <article className="doc">
        ...
        <div className="doc-body">
          {ai && (
            <Clause n={next()} title="Plain-language summary">
              <p className="doc-text">{ai.summary}</p>
              {ai.nextSteps?.length > 0 && (
                <ol className="doc-text" style={{ paddingInlineStart: 22, lineHeight: 1.9 }}>
                  {ai.nextSteps.map((stp, i) => <li key={i}>{stp}</li>)}
                </ol>
              )}
              <p className="doc-text" style={{ fontStyle: "italic", opacity: 0.8 }}>{ai.disclaimer}</p>
            </Clause>
          )}
          <Clause n={next()} title="Status of this document">
            <p className="doc-text">
              Generated by software from answers and images supplied by the person named
              nowhere in it. <strong>Not a diagnosis and not a laboratory result</strong> —
              it records what a screening tool observed and recommends. Every finding
              requires confirmation by a qualified clinician. No identifying information is
              held: the reference number above is a random string, issued only so this
              sheet can be cited in a paper register.
            </p>
            {report.anaemia && (
              <div className="doc-callout">
                <strong>For the clinician:</strong> the anaemia screen in section 3 is an
                uncalibrated colour heuristic run on a consumer phone camera, not validated
                against paired haemoglobin results in this population and tuned to
                over-refer. Treat a positive as a prompt to test, a negative as carrying no
                weight against clinical judgement.
              </div>
            )}
          </Clause>
            ...
            <table className="doc-table">
              ...
              <tbody>
                ...
                <ModuleRow
                  ...
                  outcome={report.anaemia?.severityLabel}
                />
                <ModuleRow
          ...
          <Clause n={next()} title="Recommended next steps">
            ...
            {/* The one section someone must be able to act on without reading.
                Hidden from print — a paper copy cannot speak. */}
            <div className="no-print" style={{ marginTop: 12 }}>
              <SpeakButton getText={() => buildActions(report).join(". ")} />
            </div>
          </Clause>
          ...
          <Clause
            ...
            lead="Government programmes providing the steps above free of charge — no referral required."
          >
            {schemes.map((s) => (
          ...
          <Clause
            ...
            lead="Guidance in this document is drawn from the following; departures are marked as a screening heuristic."
          >
            <table className="doc-table doc-table-accent">
              <thead>
                <tr>
                  <th>Document</th>
                  <th>Issued by</th>
              ...
              <tbody>
                {SOURCE_DOCUMENTS.map((d) => (
                  <tr key={d.title}>
                    <td style={{ fontWeight: 600, color: "var(--rose-deep)" }}>{d.title}</td>
          ...
          <Clause n={next()} title="Verification">
            <p className="doc-text">
              To be completed by the reviewing health worker. Sakhi does not countersign
              anything; this block exists as evidence a review took place.
            </p>
  ...
  return (
    <Clause n={n} title="Anaemia screening — conjunctival pallor">
      <div className={`doc-callout${a.testNeeded ? " doc-callout-urgent" : ""}`}>
        <strong>Screening conclusion: {a.severityLabel || a.severity}.</strong>{" "}
        {a.testNeeded
          ? "The colour reading crossed the screening threshold. A haemoglobin test is indicated."
          : "The colour reading did not cross the screening threshold. This does not exclude anaemia."}
      </div>
      ...
      <table className="doc-table">
        <tbody>
          <tr>
            ...
            <td>
              Conjunctival colour analysis of a single camera frame — {a.features?.roiPixelCount} px
              sampled, {Math.round((a.features?.roiRatio ?? 0) * 100)}% passed the hue/saturation filter.
            </td>
          </tr>
          <tr>
            <td className="doc-cell-key">Pallor score</td>
            <td>{a.pallorScore} of 1.00 (band: {a.severity}) — confidence {Math.round(a.confidence * 100)}%, capture quality {a.captureQuality}</td>
          </tr>
          <tr>
            <td className="doc-cell-key">Measured features</td>
            <td>
              Green-to-red ratio {a.features?.greenRedRatio}; saturation mean{" "}
              {a.features?.saturationMean}; green percentage {a.features?.greenPct}.
            </td>
          </tr>
          <tr>
            <td className="doc-cell-key">Calibration status</td>
            <td>
              Uncalibrated heuristic — literature-informed thresholds, not fitted to this
              population. Warm lighting can bias toward "healthy"; treat a negative with
 ...
 * app in — an emergency triage result must not print below a cycle note.
 */

// Compact, readable context for the AI summary. Plain lines only — no raw JSON,
// no identifiers — so the model gets what a health worker would read, nothing more.
function buildAiContext(report) {
  const lines = [];
  const lvl = report.triage?.session?.triage?.level;
  if (lvl) lines.push(`Symptom triage urgency: ${lvl}.`);
  const reason = report.triage?.session?.triage?.reason;
  if (reason) lines.push(`Triage note: ${reason}`);
  if (report.triage?.isPregnantOrPossible) lines.push("May be pregnant.");
  if (report.anaemia) {
    lines.push(`Anaemia (eyelid) screen: ${report.anaemia.severityLabel || report.anaemia.severity || "done"}${report.anaemia.testNeeded ? " — a blood test is suggested" : ""}.`);
  }
  if (report.cycle) lines.push(`Cycle/pregnancy log at week ${report.cycle.week ?? "?"}.`);
  if (lines.length === 0) lines.push("A self-check was completed with no urgent findings.");
  return lines.join("\n");
}
...
function buildActions(report) {
  ...
  if (report.triage?.isPregnantOrPossible) {
    actions.push(
...
// The two groups mirror how the main nav itself is organised (see NAV in
// Navbar.jsx) — "what do I do about symptoms" and "what do I get screened
// for" are different questions, and a flat grid of tiles answered neither.
const EMPTY_REPORT_GROUPS = [
  {
    titleKey: "nav_group_symptoms",
    titleFallback: "Check symptoms",
    tiles: [
  ...
  {
    titleKey: "nav_group_screening",
    titleFallback: "Screening",
    tiles: [
...
function EmptyReport() {
  const { t } = useLanguage();
  return (
    <div className="container" style={{ paddingTop: 44, paddingBottom: 60, maxWidth: 780 }}>
      <span className="section-eyebrow">
      ...
      <div style={{ display: "grid", gap: 34, marginTop: 26 }}>
        {EMPTY_REPORT_GROUPS.map((group) => (
          <div key={group.titleKey}>
            <h2 className="display" style={{ fontSize: 18, color: "var(--rose-deep)", marginBottom: 12 }}>
              {t(group.titleKey) === group.titleKey ? group.titleFallback : t(group.titleKey)}
            </h2>
            <div className="tile-grid">
              {group.tiles.map((tile) => (
                <Link key={tile.to} to={tile.to} className={`tile${tile.primary ? " tile-primary" : ""}`}>
                  <span className="tile-mark"><Icon name={tile.icon} size={24} /></span>
                  <span className="tile-title">{t(tile.key)}</span>
      ...
      <p style={{ fontSize: 12.5, color: "var(--ink-muted)", marginTop: 30, lineHeight: 1.7 }}>
