  ...
  return (
    <div className="container" style={{ paddingTop: 44, paddingBottom: 60, maxWidth: 680 }}>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 20, flexWrap: "wrap" }}>
        {SCENARIOS.map((s, i) => (
          <button key={s.id} onClick={() => setStep(i)} style={{
            ...
            border: i === step ? "1.5px solid #B04A63" : "1px solid #F0DFE2",
            background: i === step ? "#F2D9DF" : "white", color: i === step ? "#7E2F44" : "#6B5A5F",
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginTop: 14 }}>
        ...
        <label style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 13, color: "#6B5A5F", marginLeft: "auto" }}>
          <input type="checkbox" checked={autoPlay} onChange={(e) => setAutoPlay(e.target.checked)} />
          Auto-play
        </label>
      </div>

      <p style={{ marginTop: 16, fontSize: 13, color: "#9C8A8F" }}>
        Expected outcome: <strong style={{ color: "#7E2F44" }}>{current.expect}</strong>
      </p>

      {loading && <p style={{ marginTop: 20, color: "#9C8A8F" }}>Running triage...</p>}
